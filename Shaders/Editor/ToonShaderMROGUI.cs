...
public class ToonShaderMROGUI : ShaderGUI
{
	public override void OnGUI( MaterialEditor materialEditor, MaterialProperty[] properties )
	{
		...
		MaterialProperty mro                = FindProperty( "_MROMap", properties );
		MaterialProperty occlusion          = FindProperty( "_Occlusion", properties );
		...
		GUILayout.Box(
			"_AmbientLight can be set globally through a script or other tool. Otherwise, the ambient lighting can be "
			+ "overridden by the property below (alpha not supported)",
			helpBox );

		Color globalAmbient = Shader.GetGlobalColor( "_AmbientColor" );
		GUILayout.BeginHorizontal();
		if( ambientLightOverride ) GUI.enabled = false;
		...
		else { ambient.colorValue = new Color( ambient.colorValue.r, ambient.colorValue.g, ambient.colorValue.b, 0 ); }
		...
		GUILayout.Box( "R: Ambient Occlusion  G: Roughness  B: Metallic ", helpBox );
		materialEditor.TextureProperty( mro, "MRO" );
		materialEditor.RangeProperty( occlusion, "Occlusion (from MRO)" );
		...
		if( EditorGUILayout.Toggle( "Outline Baked Normals", bakedNormals ) ) { bakedNormalsFloat.floatValue = 1; }
		else { bakedNormalsFloat.floatValue                                                                  = 0; }

		bool equalDistance = equalDistanceFloat.floatValue > 0.5f;

		if( EditorGUILayout.Toggle( "Outline Ignore Distance", equalDistance ) ) { equalDistanceFloat.floatValue = 1; }
		else { equalDistanceFloat.floatValue                                                                     = 0; }
